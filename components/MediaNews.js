...
export default function MediaNews() {
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-baseline mb-20 border-b border-white/10 pb-8">
            <h2 className="text-7xl font-black tracking-tighter text-white">
                INSIGHTS<span className="text-brand">.</span>
            </h2>
            <div className="flex items-center gap-4 text-white/50 text-sm font-mono uppercase tracking-widest mt-6 md:mt-0">
      ...
      {/* Giant Marquee Text running behind content */}
      <div className="absolute top-1/2 left-0 w-full -translate-y-1/2 opacity-[0.03] pointer-events-none select-none">
          <div ref={marqueeInnerRef} className="whitespace-nowrap flex gap-10">
              {[...Array(4)].map((_, i) => (
                 <span key={i} className="text-[20vw] font-black leading-none text-white">
...
function SpotlightCard({ item, index }) {
    ...
    return (
        <article 
            ...
            className="group relative bg-white/5 rounded-none border border-white/10 p-8 overflow-hidden transition-colors hover:bg-white/10"
        >
            ...
            <div className="flex justify-between items-start mb-8 opacity-60">
                <span className="font-mono text-xs border border-white/20 px-2 py-1 rounded text-brand-light">
                    {item.category}
                </span>
            ...
            <div className="relative z-10 space-y-4">
                <h3 className="text-3xl font-bold text-white leading-tight group-hover:text-brand transition-colors duration-300">
                    {item.title}
                </h3>
                <p className="text-lg text-white/60 leading-relaxed border-l-2 border-white/10 pl-4 group-hover:border-brand transition-colors">
                    {item.excerpt}
                </p>
            </div>
            ...
            <div className="mt-12 flex items-center gap-4 text-white font-bold tracking-widest text-sm group/btn cursor-pointer">
                <span className="relative overflow-hidden">
                   READ ARTICLE
                   <span className="absolute bottom-0 left-0 w-full h-[1px] bg-brand transform -translate-x-100 group-hover/btn:translate-x-0 transition-transform duration-300"></span>
                </span>
...
function ViewAllNewsButton() {
  ...
  return (
    <Link href="/contact">
      <button
        ref={buttonRef}
        className="group/btn relative overflow-hidden bg-white/10 backdrop-blur-md border border-white/20 text-white px-10 py-4 rounded-full font-bold text-base tracking-widest uppercase transition-all duration-300 hover:bg-white/20 hover:border-brand/50"
      >
        {/* Glow Effect */}
        <span
          ref={glowRef}
          className="absolute inset-0 bg-brand/20 blur-xl opacity-0 transition-opacity duration-300"
        />
        ...
        {/* Animated Border */}
        <span className="absolute inset-0 rounded-full border-2 border-brand/0 group-hover/btn:border-brand/50 transition-colors duration-300" />
      </button>
    </Link>
  );
}